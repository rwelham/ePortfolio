...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done) {
        ...
        if (low >= high) {
            done = true; //the loop is done and can exit
            return high; //returns the value given
        }
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    middlePoint = partition(bids, begin, end);
    // recursively sort low partition (begin to mid)
    quickSort(bids, begin, middlePoint);
    // recursively sort high partition (mid+1 to end)
    quickSort(bids, middlePoint + 1, end);
}
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            // Initialize a timer variable before sorting with selection sort
            ticks = clock();

            selectionSort(bids); //calls selectionSort
        ...
        case 4:
            // Initialize a timer variable before sorting with quick sort
            ticks = clock();
        ...
        case 9: //case for the menu exit
            break;

        default: //exits if invalid input is given
            break;
        }
    }
