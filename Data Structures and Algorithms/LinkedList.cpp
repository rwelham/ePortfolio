...
void LinkedList::Prepend(Bid bid) {
    ...
    else { //there's nothing in the list so the new item can be appended
        Append(bid);
    }
    //increase size count
    size++;
...
void LinkedList::PrintList() {
    ...
    // start at the head
    Node* currNode = head;
    // for loop over each node looking for a match
...
void LinkedList::Remove(string bidId) {
    ...
    for(int i = 0; i < size - 1; i++) {
        ...
        else if ((currNode->next->bid.bidId == bidId) && (currNode->next->next == nullptr)) {
            //hold onto the next node temporarily
            tempNode = currNode->next;
            //make tail current node
            tail = currNode;
            //make the next pointer past current node null
